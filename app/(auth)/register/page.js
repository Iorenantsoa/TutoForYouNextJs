...
function Register() {
  ...
  const [info, setInfo] = useState({ name: "", contact: "", email: "", password: "", confirm_password: "" })
  ...
  const handleSubmit = async (e) => {
    e.preventDefault() 
    if (!info.name || !info.email || !info.contact || !info.password || !info.confirm_password) {
      setError({message  : "Tous les champs sont obligatoires" , success : false})
      toast.error("Tous les champs sont obligatoires")
      return
    }
    if (info.password !== info.confirm_password) { 
      setError({message  : "Les mot de passes ne se correspondent pas" , success : false})
      toast.error("Les mot de passes ne se correspondent pas")
      return
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push('/login')
        toast.success("Vous êtes enregistré avec succees") 
      } else {
    ...
    <div>
      <div className='relative top-0 left-0 right-0'>
        ...
        <div className='h-full w-full   bg-slate-50 ' style={{ minHeight: "70vh" }}>
          <div className='relative -top-36 w-11/12 md:w-1/4 mx-auto'>
            <div className='max-w-sm  mx-auto'>
              ...
              <div className=' bg-slate-50 shadow-md py-4  rounded'>
                <form className='mx-4' onSubmit={handleSubmit}>
                  <div className='mb-3'>
                    <h2 className=' text-blue-800 text-xl md:text-2xl font-bold  '>S{"'"}inscrire</h2>
                  </div>
                  ...
                  <div className="relative z-0 w-full mb-5 group">
                    ...
                    <label htmlFor="confirm_password" className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Confirmer votre mot de passe</label>
                  </div>
                  ...
                  :
                  (
                    <button type="submit" className="text-white bg-blue-800 hover:bg-blue-900 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm  w-full   px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">S{"'"}inscrire</button>
                  ) 
                }
              ...
              <div className='flex justify-center '>
                <span className=' text-gray-900 mt-3 text-sm  '>Vous êtes déjà inscrit(e) ? <Link href="/login" className='text-orange-500 font-semibold'>Se connecter ici !</Link></span>
              </div>
            </div>
